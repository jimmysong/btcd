...
func TestScalarMult2(t *testing.T) {
	// From btcd issue #709
	x := new(big.Int).SetBytes([]byte{0x2C})
	y := new(big.Int).SetBytes([]byte{0x42, 0x0E, 0x7A, 0x99, 0xBB, 0xA1, 0x8A, 0x9D, 0x39, 0x52, 0x59, 0x75, 0x10, 0xFD, 0x2B, 0x67, 0x28, 0xCF, 0xEA, 0xFC, 0x21, 0xA4, 0xE7, 0x39, 0x51, 0x09, 0x1D, 0x4D, 0x8D, 0xDB, 0xE9, 0x4E})
	kData := []byte{0xA2, 0xE8, 0xBA, 0x2E, 0x8B, 0xA2, 0xE8, 0xBA, 0x2E, 0x8B, 0xA2, 0xE8, 0xBA, 0x2E, 0x8B, 0xA2, 0x19, 0xB5, 0x18, 0x35, 0xB5, 0x5C, 0xC3, 0x0E, 0xBF, 0xE2, 0xF6, 0x59, 0x9B, 0xC5, 0x6F, 0x58}
	s256 := btcec.S256()
	xGot, yGot := s256.ScalarMult(x, y, kData)
	xWant := new(big.Int).SetBytes([]byte{0xDA, 0x09, 0xBD, 0x5D, 0x57, 0x4E, 0x1B, 0x17, 0x52, 0xFF, 0x30, 0x29, 0xC7, 0x6B, 0x32, 0x46, 0x38, 0x10, 0x82, 0x51, 0x35, 0xB9, 0xCC, 0x11, 0xD5, 0xB9, 0x0B, 0x75, 0x5A, 0xFC, 0x5E, 0x7F})
	yWant := new(big.Int).SetBytes([]byte{0x3E, 0x61, 0xDE, 0xF0, 0x8C, 0x03, 0x5F, 0xFC, 0x5C, 0xA9, 0x43, 0xA2, 0x15, 0x11, 0x30, 0x80, 0x92, 0x1B, 0xC3, 0x52, 0xC4, 0xD1, 0x24, 0x1D, 0xAE, 0x92, 0xD3, 0x99, 0x1B, 0x34, 0x4D, 0x87})
	if xGot.Cmp(xWant) != 0 || yGot.Cmp(yWant) != 0 {
		t.Fatalf("bad output: got (%X, %X), want (%X, %X)", xGot, yGot, xWant, yWant)
	}
}
